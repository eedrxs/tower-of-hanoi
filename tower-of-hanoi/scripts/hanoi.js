...
const hanoi = {
  ...
  renderNext() {
    const template = new Template("", null, this.buffer[this.pos]);
    const message = template.tags.info;
    const arrangement = this.prevRender.slice(
      this.prevRender.indexOf("</div>") + 6
    );
    const container = document.createElement("div");

    container.className = "container";
    container.insertAdjacentHTML("afterbegin", message + arrangement);
    const disk = container
      .querySelector(`.${this.buffer[this.pos].from}`)
      .querySelector(`.disk-${this.buffer[this.pos].disk}`);

    if (
      !container
        .querySelector(`.${this.buffer[this.pos].to}`)
        .querySelector('div[class^="disk"]')
    ) {
      const div = container
        .querySelector(`.${this.buffer[this.pos].to}`)
      ...
      const div = container
        .querySelector(`.${this.buffer[this.pos].to}`)
        .querySelectorAll('div[class^="disk"]');
      div[0].parentElement.parentElement.previousElementSibling.children[1].firstElementChild.className =
        disk.className;
      div[0].parentElement.parentElement.previousElementSibling.children[1].firstElementChild.innerText =
        disk.innerText;
    }
  ...
  renderInitial() {
    ...
    const template = new Template(" ", this.disks);
  ...
  towerOfHanoi(n, from_rod, to_rod, aux_rod) {
    if (n == 1) {
      this.buffer[this.move] = new this.Iteration(
        ++this.move,
        1,
        from_rod,
        to_rod
